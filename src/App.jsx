...
function ExploreMosaic({ pilotMarkets, visible, setExploreAll }) {
  const pilotIds = new Set(pilotMarkets.map((market) => market.id));
  const otherMarkets = visible.filter((market) => !pilotIds.has(market.id)).slice(0, 3);
  return <section className="explore-mosaic" id="explore-highlights"><div className="explore-visual"><img src="/featured-food.jpg" alt="夜市美食意象示意圖片" /><span>夜市美食意象・示意圖片</span></div><div className="explore-places"><span className="section-label">從桃園開始</span><h2>先逛中壢與中原</h2>{pilotMarkets.map((market) => <a className="explore-place" key={market.id} href={"/markets/" + encodeURIComponent(market.id)}><span><b>{market.name}</b><small>{market.city} · {market.district} · {market.address || "地址待補充"}</small></span><ArrowRight size={16} /></a>)}</div><div className="explore-nearby"><span className="section-label">可探索入口</span><h2>再探索幾個夜市</h2>{otherMarkets.map((market) => <a key={market.id} href={"/markets/" + encodeURIComponent(market.id)}><b>{market.name}</b><small>{market.city} · {market.district}</small></a>)}<button className="text-button" onClick={() => { setExploreAll(true); document.getElementById("registry-explorer")?.scrollIntoView({ behavior: "smooth" }); }}>探索全台名錄 <ArrowRight size={14} /></button></div></section>;
}
