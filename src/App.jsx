...
function ExploreMosaic({ pilotMarkets, visible, markets, cities, setExploreAll }) {
  ...
  return <section className="explore-mosaic" id="explore-highlights"><div className="mosaic-heading"><div><span className="section-label">圖片導覽</span><h2>從熟悉的夜晚開始探索</h2><div className="mosaic-meta"><span>{markets.length} 筆夜市</span><span>{cities.length} 個縣市</span><span>{markets.filter((market) => market.latitude != null).length} 個可定位點位</span></div></div><p>夜市意象圖為版面示意，不代表特定夜市實景。</p></div><div className="mosaic-grid">{tiles.map((market, index) => <a className={`explore-tile explore-tile-${index + 1}`} key={market.id} href={"/markets/" + encodeURIComponent(market.id)}><MarketImagePlaceholder className="explore-tile-placeholder" photoUrl={market.photoUrl} source={market.photoSource || market.sourceUrl} alt="夜市意象圖" /><span className="explore-tile-scrim" /><span className="explore-tile-copy"><b>{market.name}</b><small>{market.city} · {market.district}</small></span></a>)}</div><button className="mosaic-more" onClick={() => { setExploreAll(true); document.getElementById("registry-explorer")?.scrollIntoView({ behavior: "smooth" }); }}>探索全台名錄 <ArrowRight size={14} /></button></section>;
}
