...
function App() {
  ...
  return <div className="registry-app">
    <header className="registry-header"><a className="registry-brand" href="/"><span><MapPin size={20} /></span><b>夜市社群</b><small>全台夜市資料庫</small></a><label className="header-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜尋夜市" /></label><button className="header-locate" onClick={locate}><LocateFixed size={15} /> 我的附近</button><div className="header-source"><Database size={15} /> 經濟部資料 <a href="https://data.gov.tw/dataset/95760" target="_blank" rel="noreferrer">95760</a></div>{!supabaseConfigured ? <span className="header-readonly">唯讀名錄</span> : user && !isAnonymousUser(user) ? <div className="header-member"><span><b>會員</b><small>{user.email}</small></span><button onClick={signOut}><LogOut size={14} /> 登出</button></div> : <button className="header-auth" onClick={focusAuth}><LogIn size={15} /> 登入參與表決</button>}<button className="header-contribute" disabled={!supabaseConfigured} onClick={() => setShowContribution(true)}><Plus size={16} /> 新增提案</button></header>
    <main className="registry-main"><section className="registry-intro"><div><div className="eyebrow">NIGHT MARKET COMMUNITY</div><h1>先找到你附近的夜市。</h1><p>一起補充、一起確認，讓每個夜市都更好逛。</p></div><div className="location-controls"><button className="locate-button" onClick={locate}><LocateFixed size={16} className={locating ? 'spin' : ''} /> {locating ? '定位中…' : '使用我的位置'}</button><label><span>選擇縣市</span><select value={city} onChange={(event) => setCity(event.target.value)}><option value="全部">全台</option>{cities.map((item) => <option key={item}>{item}</option>)}</select></label></div></section><div id="community-auth"><AuthPanel user={user} authBusy={authBusy} authMessage={authMessage} email={email} setEmail={setEmail} onSendMagicLink={sendMagicLink} onSignOut={signOut} authOpen={authOpen} setAuthOpen={setAuthOpen} /></div><MyQualityReports user={user} /><AdminPanel role={role} busy={proposalBusy} onModerate={moderateProposal} onQualityModerate={moderateQualityReport} />{showContribution && !selected && <section className="standalone-contribution"><ContributionForm selected={null} user={user} marketDbId={null} busy={proposalBusy} onSubmit={submitProposal} onClose={() => setShowContribution(false)} /></section>}
